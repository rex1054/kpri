...
if(isset($_GET['m'])){
	...
	$filter = "WHERE YEAR(peminjaman.`mulai`) = '".$bulan[0]."' AND Month(peminjaman.`mulai`) = '".$bulan[1]."' AND ajuan.peminjam = ".$_SESSION['nip'];
} else if(isset($_GET['y'])){
	$tahun = $_GET['y'];
	$filter = "WHERE YEAR(peminjaman.`mulai`) = '".$tahun."' AND ajuan.peminjam = ".$_SESSION['nip'];
} else { $filter = "WHERE ajuan.peminjam = ".$_SESSION['nip']; }
...
if($query->num_rows == 0) {}
	...
	if(mysqli_num_rows($query)>0) { 
		while ($data = mysqli_fetch_array($query)) {
			...
			<td><a href="<?php echo $siteurl.'anggota/detil-pinjaman/?id='.$data['id']; ?>"><u><?php echo $data["nama"];?></u></a></td>
